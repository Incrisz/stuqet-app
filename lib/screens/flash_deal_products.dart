...
import 'package:active_ecommerce_flutter/data_model/product_mini_response.dart' as productMini;
...
class _FlashDealProductsState extends State<FlashDealProducts> {
  ...
  getInfo()async{
    ...
    if(res.flashDeals?.isNotEmpty??false) {
      ...
      DateTime end = convertTimeStampToDateTime(flashDealInfo!.date!); // YYYY-mm-dd
  ...
  void initState() {
    ...
    getInfo();
    _future =
        ProductRepository().getFlashDealProducts(widget.slug);
    _searchList = [];
    _fullList = [];
    super.initState();
  }
  ...
  buildProductList(context) {
    return FutureBuilder(
        future: _future,
        builder: (context,AsyncSnapshot<productMini.ProductMiniResponse> snapshot) {
